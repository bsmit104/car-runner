...
public class CarSpawner : MonoBehaviour {    public GameObject[] carPrefabs; // Array of car prefabs to spawn
    ...
    private int lanes = 10;               // Number of lanes (0: Left, 1: Center, 2: Right)
    ...
    IEnumerator SpawnCars() {
        while (true) {
            ...
            int carsToSpawn = Random.Range(1, 4); // Spawn 1-3 cars at a time
            for (int i = 0; i < carsToSpawn; i++) {
                if (openLanes.Count == 0) break;

                int laneIndex = Random.Range(0, openLanes.Count);
                int lane = openLanes[laneIndex];
                openLanes.RemoveAt(laneIndex);

                Vector3 spawnPos = new Vector3(laneDistance * (lane - 5), 0, player.position.z + spawnDistance);
